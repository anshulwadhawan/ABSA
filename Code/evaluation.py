...
def prepareJsonLines(pred_data_dir):
...
#    df['trueLabels'] = y_true
    ...
    for i in range(len(df)):
        trueLabel = df['label'][i]
        idd = df['id'][i]
        sentence = df['sentence1'][i]
        aspectandentity = df['sentence2'][i]
        predictedLabel = df['predictedLabels'][i]
        
        if trueLabel != "None":
            opinion={}
            opinion['sentiment'] = trueLabel
            opinion['aspect'] = aspectandentity[15:]
            opinion['target_entity'] = aspectandentity[:12]
            opinions.append(opinion)
        
        if predictedLabel != "None":
            pred={}
            pred['sentiment'] = predictedLabel
            pred['aspect'] = aspectandentity[15:]
            pred['target_entity'] = aspectandentity[:12]
            model_pred.append(pred)
        
        if trueLabel == predictedLabel:

            if trueLabel == "Positive" :
                if aspectandentity[15:] == "general":
                    correctgeneralpos = correctgeneralpos + 1
                elif aspectandentity[15:] == "price":
                    correctpricepos = correctpricepos + 1
                elif aspectandentity[15:] == "safety":
                    correctsafetypos = correctsafetypos + 1
                else:
                    correcttransitpos = correcttransitpos + 1
            
            else :
                if aspectandentity[15:] == "general":
                    correctgeneralneg = correctgeneralneg + 1
                elif aspectandentity[15:] == "price":
                    correctpriceneg = correctpriceneg + 1
                elif aspectandentity[15:] == "safety":
                    correctsafetyneg = correctsafetyneg + 1
                else:
                    correcttransitneg = correcttransitneg + 1
            

        if trueLabel != predictedLabel:
            if aspectandentity[15:] == "general":
                wronggeneral = wronggeneral + 1
            elif aspectandentity[15:] == "price":
                wrongprice = wrongprice + 1
            elif aspectandentity[15:] == "safety":
                wrongsafety = wrongsafety + 1
            else:
                wrongtransit = wrongtransit + 1
        

        
        if i==len(df)-1 or df['id'][i]!=df['id'][i+1]:
            myjson={}
            myjson['opinions'] = opinions
            myjson['id'] = str(idd)
            myjson['text'] = sentence
            myjson['model_pred'] = model_pred
            
            myjsonlines.append(myjson)
            
            opinions=[]
            model_pred=[]


    # with jsonlines.open('output_5epochs.jsonl', mode='w') as writer:
    #     writer.write_all(myjsonlines)

    print(myjsonlines)

    print(correctgeneralpos)
    print(correctpricepos)
    print(correctsafetypos)
    print(correcttransitpos)
    print(correctgeneralneg)
    print(correctpriceneg)
    print(correctsafetyneg)
    print(correcttransitneg)
    print(wronggeneral)
    print(wrongprice)
    print(wrongsafety)
    print(wrongtransit)
